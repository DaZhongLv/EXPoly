...
def _ball_grid(radius: float, step: float, random_center: bool, rng: Optional[np.random.Generator]) -> np.ndarray:
    """
    ...
    cube_struct = np.array([[i, j, k] for i in one_dim for j in one_dim for k in one_dim], dtype=float)

    tree = spatial.cKDTree(cube_struct)
    center = (rng.random(3) * 2.0) if (random_center and rng is not None) else (np.array([0.0, 0.0, 0.0]))
...
def prepare_carve(
    ...
    rng = np.random.default_rng(cfg.rng_seed) if cfg.rng_seed is not None else np.random.default_rng()
    t0 = time.process_time()
    sc_points = _ball_grid(crave_r, cfg.ratio, cfg.random_center, rng)
    logger.info("prepare_carve: SC sphere in %.3fs (points=%d)", time.process_time() - t0, len(sc_points))
...
def carve_points(
    ...
    logger.info("carve_points: kept %d/%d in %.3fs", len(kept), len(tr_pts), time.process_time() - t0)
    return kept
